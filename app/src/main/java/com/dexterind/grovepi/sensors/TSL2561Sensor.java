...
public class TSL2561Sensor {
    ...
    public final static int TSL2561_ADDRESS = 0x29;
    ...
    public TSL2561Sensor(int address)
    {
        try {
            manager = new PeripheralManagerService();

            // Get device itself
            tsl2561 = manager.openI2cDevice(I2C_DEVICE_NAME, address);
    ...
    {
        tsl2561.writeRegByte(TSL2561_COMMAND_BIT, (byte)TSL2561_CONTROL_POWERON);
    }

    public void turnOff() throws IOException
    {
        tsl2561.writeRegByte(TSL2561_COMMAND_BIT, (byte)TSL2561_CONTROL_POWEROFF);
    }
    ...
    public void setGain(int gain, int integration) throws IOException
    {
        ...
        {
            tsl2561.writeRegByte(TSL2561_COMMAND_BIT | TSL2561_REGISTER_TIMING, (byte)(gain | integration));
            Log.d(TAG, "Setting low gain");
            this.gain = gain;
    ...
    public double readLux() throws Exception {
        ...
        Log.d(TAG, "IR Result:" + ir);
        Log.d(TAG, "Ambient Result:" + ambient);
    /*
    ...
    private int readU8(int reg) throws Exception
    {
        ...
        try
        {
            result = this.tsl2561.readRegByte(reg);
            //Log.d(TAG, "(U8) I2C: Device " + toHex(TSL2561_ADDRESS) + " returned " + toHex(result) + " from reg " + toHex(reg));
        }
        catch (Exception ex)
        {
            ex.printStackTrace();
    ...
    private int readU16(int register) throws Exception
    {
        ...
        //Log.d(TAG, "(U16) I2C: Device " + toHex(TSL2561_ADDRESS) + " returned " + toHex(result) + " from reg " + toHex(register));
        return result;
    }
